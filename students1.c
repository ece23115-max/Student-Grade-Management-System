...
// Tree-traversal search because the BST keys are CGPAs, not names
...
// Function to read data from CSV file (Now handles column headers)
int readCSVData(const char *filename, Student students[], int *num_students) {
    ...
    // UPDATED: Read and skip the first line (the header row)
    ...
    int count = 0;
    while (fgets(line, 1024, file) && count < 100) { 
        Student student;
        ...
        if (fields_read != 7) {
            fprintf(stderr, "Error reading line %d. Skipping.\n", count + 2); // +2 accounts for header and 1-based index
            continue;
        }
